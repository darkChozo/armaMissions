...
if (hasInterface) then {
	...
	[
		...
		"a3\ui_f\data\IGUI\cfg\simpleTasks\types\repair_ca.paa",
		"_this != vehicle _this && vehicle _this distance repairbay < 6",
		"_this != vehicle _this && vehicle _this distance repairbay < 6",
		{},
		{},
		{
			["Alert",["Service Complete!"]] call BIS_fnc_showNotification;
			[vehicle (_this select 1),1] remoteExec ["setFuel",vehicle (_this select 1)];
			[vehicle (_this select 1),0] remoteExec ["setDamage",vehicle (_this select 1)];
			[vehicle (_this select 1),1] remoteExec ["setVehicleAmmo",vehicle (_this select 1)];
		},
		{ ["Alert",["Service Interrupted!"]] call BIS_fnc_showNotification },
		nil,
		30,
		6
