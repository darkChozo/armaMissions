...
_localInit = {
	...
	if (isNil {_unit getVariable "f_wound_localInit"}) then {
		_unit setVariable ["f_wound_down",false];
		_unit setVariable ["f_wound_bleeding",false];
		_unit addEventHandler ["HandleHeal",{_this call f_fnc_OnHeal}];
		_unit setVariable ["f_wound_healInit",true];
			// Drag Action.
		_addIndex = _unit addAction [format ["Drag %1", name _unit],{[_this, "f_fnc_OnDrag", [_this select 0,_this select 1],false] spawn BIS_fnc_MP;}, nil, 6, false, true, "", "_var = _this getVariable ['f_wound_dragging',nil];_target distance _this < 2 && isNil '_var' && _target getVariable['f_wound_down',false] && !(_this getVariable ['f_wound_down',false])"];
		_unit setVariable ["f_wound_localInit",true];
	};
};
{ [_x] call _localInit } foreach playableUnits;
addMissionEventHandler ["EntityRespawned",_localInit];
