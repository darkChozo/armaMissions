...
if (isServer) then {
    sleep .01;
    waitUntil {{alive _x} count playableUnits <= 1};
    if ({alive _x} count playableUnits == 1) then {
        ["Alert",[format ["%1 is the last one left alive!",name (playableUnits select 0)]]] remoteExec ["BIS_fnc_showNotification"];
    };
    waitUntil {{alive _x} count playableUnits == 0};
    [1] call f_fnc_mpEnd;
};
