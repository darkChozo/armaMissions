...
[_unit, _medicalSystem, _damageMultiplier, _crewPostCrashCode] spawn {
	...
	_faks = [];
	{
		if (_x isKindOf ["FirstAidKit",configFile >> "CfgWeapons"]) then {
			_faks pushBack _x;
			_unit removeItem _x;
		};
	} forEach items _unit;
	_unit setVariable ["r0ed_SurvivableCrashes_faks",_faks];
    ...
    _crewSpawn = [_unit] spawn _crewPostCrashCode;
	waitUntil {sleep 1; scriptDone _crewSpawn};
	{ _unit addItem _x } forEach _faks;
};
