...
if (isServer) then {
    _handle = [] execVm "ws_garrisonControl.sqf";
    waitUntil {scriptDone _handle};
    _potentialHostages = ["hostage"] call ws_fnc_collectObjects;
    _hostages = [];
    for "_i" from 1 to 3 do {
        _thostage = _potentialHostages call BIS_fnc_selectRandom;
        _potentialHostages = _potentialHostages - [_thostage];
        _hostages pushBack _thostage;
    };
    ...
    hostages = [];
    {
        ...
        } forEach nearestObjects [_x,["Man"],13];
        hostages append units group _x;
    } forEach _hostages;
};
...
waitUntil {!isNil "hostages"};

f_var_removeBodyDelay = 120;
f_var_removeBodyDistance = 360;
f_var_doNotRemoveBodies = hostages;
