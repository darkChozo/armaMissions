...
// BLUFOR > NATO
[BLUFOR,100,{[] execVM "endMission.sqf"}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";

// OPFOR > CSAT
// [OPFOR,100,1] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";

// INDEPENDENT > AAF
[INDEPENDENT,100,{[] execVM "endMission.sqf"}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";
...
if (hasInterface) then {
    ...
    player setVariable ["suppliesCount",0,true];
    player addAction ["Take supplies",{
        player playAction "PutDown";
        player setVariable ["suppliesCount",1,true];
        cursorObject setVariable ["suppliesCount",(cursorObject getVariable "suppliesCount") - 1,true];
        hint "Supplies carried: 1";
    },nil,7,true,true,"","!isNull cursorObject && {!(cursorObject in [aafdropoff,fiadropoff]) && {player distance cursorObject < 4 && player getVariable 'suppliesCount' == 0 && !isNil {cursorObject getVariable ['suppliesCount',nil]} && {cursorObject getVariable 'suppliesCount' > 0}}}"];

    player addAction ["Put supplies in vehicle",{
        player playAction "PutDown";
        player setVariable ["suppliesCount",0,true];
        if (isNil {cursorObject getVariable "suppliesCount"}) then { cursorObject setVariable ["suppliesCount",0] };
        cursorObject setVariable ["suppliesCount",(cursorObject getVariable "suppliesCount") + 1,true];
        hint "Supplies carried: 0";
    },nil,6,true,true,"","!isNull cursorObject && {player distance cursorObject < 4 && player getVariable 'suppliesCount' == 1 && cursorObject isKindOf 'LandVehicle'}"];

    player addAction ["Check for supplies",{
        player playAction "PutDown";
        titleText [format ["Contains %1 supplies",cursorObject getVariable "suppliesCount"],"PLAIN",1];
    },nil,2,true,true,"","!isNull cursorObject && {player distance cursorObject < 4 && !isNil {cursorObject getVariable ['suppliesCount',nil]}}"];

    [] spawn {
        while {true} do {
            hint format ["Supplies carried: %1",player getVariable "suppliesCount"];
            sleep 5;
        };
    };
    ...
    sleep 3;
    ['"Beer Run"',"Northwestern Altis"] call ws_fnc_showIntro;
};
