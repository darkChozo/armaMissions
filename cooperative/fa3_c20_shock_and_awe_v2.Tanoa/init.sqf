...
if (hasInterface) then {
	...
	if (faction player == "blu_f") then {
		addMissionEventHandler ["Draw3d",{
			if (vehicle player == VehVTOL) then {
				{
					if (faction _x == "blu_ctrg_F") then {
						drawIcon3d ["a3\ui_f\data\IGUI\Cfg\Cursors\select_ca.paa",[.2,.2,.8,.4],_x modelToWorld [0,0,1],1,1,0];
					};
				} forEach playableUnits;
				{
					drawIcon3d ["a3\ui_f\data\IGUI\Cfg\Cursors\select_ca.paa",[.2,.2,.8,.4],_x modelToWorld [0,0,0],1,1,0];
				} forEach [lsv1,lsv2,lsv3,lsv4];
			};
		}];
	};
	sleep 10;
	['"Shock and Awe"',"Tanoa, Horizon Islands"] call ws_fnc_showIntro;
};
