...
f_var_mapClickTeleport_Units = [];
{
	if (side _x == independent) then {
		f_var_mapClickTeleport_Units pushBack _x;
	}
...
[BLUFOR,100,{
	if (isServer) then {
		if (capturePoints >= targetCapturePoints) then {
			[2] call f_fnc_mpEnd;
		} else {
			[3] call f_fnc_mpEnd;
		};
	};
}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";
...
if (isServer) then {
	{
		_mkr = createMarker [format ["%1",_x],_x modelToWorld boundingCenter _x];
		...
		_mkr setMarkerSize [.5* abs ((_bb select 1 select 0) - (_bb select 0 select 0)),.5 * abs ((_bb select 1 select 1) - (_bb select 0 select 1))];
	} forEach allmissionObjects "House";

	_mkr = createMarker ["actualobjmarker",getMarkerPos "objmarker"];
	_mkr setMarkerShape "ICON";
	_mkr setMarkerType "mil_objective";
	_mkr setMarkerText "CSAT Black Site";
	
	
	[] spawn {
		targetCapturePoints = 2400;
		capturePoints = 0;
		waitUntil {!isNil {objcapture}};
		while {capturePoints < targetCapturePoints} do {
			if (objcapture) then {
				capturePoints = capturePoints + 5;
				"Uploading..." remoteExec ["hintSilent",west];
			} else {
		...
		"" remoteExec ["hintSilent",west];
		["Alert",["Objective complete! Time to leave."]] remoteExec ["BIS_fnc_showNotification",west];
	};
};
...
if (hasInterface) then {
	...
	if (side player != independent) then {
		{ deleteMarkerLocal _x } foreach ["tz1","tz2","tz3"];
	};
};
