...
{ if (side _x == independent) then {f_var_mapClickTeleport_Units pushBack _x}} forEach playableUnits;
...
if (isServer) then {
	doubleCrossSides = [];
	if (random 1 > .5) then {doubleCrossSides pushBack east};
	if (random 1 > .5) then {doubleCrossSides pushBack west};
	publicVariable "doubleCrossSides";
};
...
if (hasInterface) then {
	...
	if (side player != independent) then {
		{ deleteMarkerLocal _x } forEach ["teleZone1","teleZone2"];
	};
	sleep .1;
