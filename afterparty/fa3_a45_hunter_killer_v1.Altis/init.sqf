...
if (isServer) then {
    ...
    _dests = ["dest"] call ws_fnc_collectMarkers;
    _index = floor random count _spawns;
    
    for "_i" from 0 to (count _spawns) - 1 do {
        systemChat format ["%1 - %2",_i,markerPos (_spawns select _i) distance2d markerPos (_dests select _i)];
    };
    
    civSpawn = _spawns select _index;
    civDest = _dests select _index;
    publicVariable "civSpawn";
    publicVariable "civDest";

    [_spawns,_index] spawn {
        params ["_spawns","_index"];
        sleep .01;
        {
            if (side _x == civilian) then {
                attemptedEscapees = attemptedEscapees + 1;
                _x setDir markerDir (_spawns select _index);
                _x setPos ([markerPos (_spawns select _index),
                           _x distance markerPos "civCenter",
                           ([markerPos "civCenter",_x] call BIS_fnc_dirTo) + markerDir (_spawns select _index)] call BIS_fnc_relPos);
            };
        } forEach playableUnits;
        
        waitUntil {{side _x == civilian} count playableUnits == 0};
        ["Alert",[format ["%1/%2 civilians escaped",escapees,attemptedEscapees]]] remoteExec ["BIS_fnc_showNotification"];
        sleep 2;
        [1] call f_fnc_mpEnd;
    };
};
...
if (hasInterface) then {
    waitUntil {!isNull player && !isNil {civSpawn} && !isNil {civDest}};
    player setDir markerDir civSpawn;
    if (side player == civilian) then {
        civDest setMarkerShapeLocal "ELLIPSE";
        civDest setMarkerBrushLocal "Solid";
        civDest setMarkerSizeLocal [10,10];
        civDest setMarkerColorLocal "ColorCIV";
        createMarker ["civDestText",markerPos civDest];
        "civDestText" setMarkerShapeLocal "ICON";
        "civDestText" setMarkerTypeLocal "hd_objective";
        "civDestText" setMarkerColorLocal "ColorCIV";
        "civDestText" setMarkerTextLocal "Safehouse";
        "searchArea" setMarkerAlphaLocal 0;
    };
    [] spawn {
        waitUntil {player distance markerPos civDest < 10};
        [{escapees = escapees + 1;}] remoteExec ["BIS_fnc_call",2];
        ["Alert",["You escaped!"]] call BIS_fnc_showNotification;
        player setVehiclePosition [markerPos "ultrasafehouse",[],0,"NONE"];
        [player,objNull,nil,nil,true] call f_fnc_camInit;
    };
};