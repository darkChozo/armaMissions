...
if (isServer) then {
	[independent,4*({side _x == west} count playableUnits)] call BIS_fnc_respawnTickets;
	
	[] spawn {
		// after 40 minutes, remove remaining reinforcements
		sleep (40 * 60);
		["Alert",["The boss isn't happy. Finish it now or not at all!"]] remoteExec ["BIS_fnc_showNotification",independent]; 
		[independent,-([independent] call BIS_fnc_respawnTickets)] call BIS_fnc_respawnTickets;
	};
};

addMissionEventHandler ["EntityKilled",{
    if (local (_this select 0)) then {
        [_this select 0] joinSilent createGroup independent;
        _this select 0 setCaptive false;
    };
}];

addMissionEventHandler ["EntityRespawned",{
    if (!isNull (_this select 1) && local (_this select 0)) then {
		_roll = random 1;
		if (_roll < .1) then {
			["rat",_this select 0,"ind_c_f"] call f_fnc_assignGear;
		};
		if (_roll > .1 && _roll < .2) then {
			["ar",_this select 0,"ind_c_f"] call f_fnc_assignGear;
		};
		if (_roll > .2) then {
			["r",_this select 0,"ind_c_f"] call f_fnc_assignGear;
		};
        [_this select 0] joinSilent createGroup independent;
        _this select 0 setCaptive false;
    };
...
[] spawn {while {true} do {
	if (side player == independent) then {
		hint format ["Reinforcements left: %1",[independent] call BIS_fnc_respawnTickets];
	};
	sleep 2;
}};
