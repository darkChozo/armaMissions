...
if (hasInterface) then {
    ...
    player addAction ["Force take uniform",{
        _uniform = nil;
        _items = [];
        {
        ...
        } forEach itemCargo cursorTarget;
        clearItemCargoGlobal cursorTarget;
        {
            cursorTarget addItemCargoGlobal [_x,1];
        } forEach _items;
        player forceAddUniform _uniform;
    },nil,6,false,true,"","{_x isKindOf ['Uniform_Base',configFile >> 'CfgWeapons']} count itemCargo cursorTarget == 1"];
};
