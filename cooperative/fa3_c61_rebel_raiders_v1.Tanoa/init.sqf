...
[] execVM "enemyTerritory.sqf";

if (isServer) then {
	[] spawn {
		_mpVics = ["mpVic"] call ws_fnc_collectObjects;
		waitUntil {sleep 1; {alive _x} count _mpVics <= .2 * count _mpVics};
		motorPoolDestroyed = true;
		["Alert",["Motor Pool Destroyed"]] remoteExec ["BIS_fnc_showNotification"];
	};
	[] spawn {
		_mortars = [mortar] call ws_fnc_collectObjectsNum;
		{
		...
		waitUntil {sleep 1; {alive _x} count _mortars == 0};
		mortarsDestroyed = true;
		["Alert",["Mortars Destroyed!"]] remoteExec ["BIS_fnc_showNotification"];
	};
};

sleep 10;
["Comms Bravo","Tanoa, the Horizon Islands"] call ws_fnc_showIntro;
