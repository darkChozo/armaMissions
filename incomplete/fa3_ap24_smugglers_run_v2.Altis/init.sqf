...
if (hasInterface) then {
	...
	if (side player == independent) then {
		"ao" setMarkerAlphaLocal 0;
		_dzs = ["dz"] call ws_fnc_collectObjects;
		_missionFailed = false;
		_smoke = "SmokeShellGreen_Infinite" createVehicleLocal [0,0,0];
		_dzMkr = createMarkerLocal ["dzMkr",[0,0,0]];
		_dzMkr setMarkerShapeLocal "ICON";
		_dzMkr setMarkerTypeLocal "mil_destroy";
		_dzMkr setMarkerColorLocal "ColorBlack";
		
		_direcitonMarker = createMarker ["directionMarker",[0,0,0]];
		_direcitonMarker setMarkerShapeLocal "ICON";
		_direcitonMarker setMarkerTypeLocal "mil_arrow";
		addMissionEventHandler ["EachFrame", {
			_dist = 80 + 0*(abs speed player) + .025*((abs speed player)^2);
			if (!isNil {dz} && {_dist + 95 < player distance2d dz}) then {
				_dir = [player,dz] call BIS_fnc_dirTo;
				"directionMarker" setMarkerDirLocal _dir;
				"directionMarker" setMarkerPosLocal ([player,_dist,_dir] call BIS_fnc_relPos);
				"directionMarker" setMarkerAlphaLocal 1;
			} else {
				"directionMarker" setMarkerAlphaLocal 0;
			};
			hintSilent format ["Time left: %1 seconds",(floor ((targetTime - time max 0) *100))/100];
		}];
		
		while {!_missionFailed} do {
			dz = selectRandom _dzs;
			targetTime = 45 + time + ( ((player distance dz) / 16) * (1 - (time / 4000)) );
			_dzMkr setMarkerPosLocal position dz;
			_smoke setPos position dz;
			waitUntil {sleep .25; vehicle player distance2d dz < 5 || time > targetTime};
			if (time > targetTime) then {
				vehicle player setVelocity [75 * sin getDir vehicle player,75 * cos getDir vehicle player,75];
				["Alert",["YOU FAIL"]] call BIS_fnc_showNotification;
				_missionFailed = true;
			} else {
				["Alert",["Drop Secured"]] call BIS_fnc_showNotification;
			};
		};
		
		sleep 30;
		player setDamage 1; // just in case
	};
};
