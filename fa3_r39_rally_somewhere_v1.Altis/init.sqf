...
lastCheckpoint = position player;
[] execVM "vehicleDoors.sqf";


if (isServer) then {
    sleep 2;
    startTime = serverTime + 20;
    publicVariable "startTime";
};

waitUntil {!isNil {startTime}};
["Alert",[format ["Race starts in %1 seconds",round (startTime - serverTime)]]] call BIS_fnc_showNotification;
waitUntil {serverTime >= startTime};
["Alert",["GO"]] call BIS_fnc_showNotification;
