...
// BLUFOR > NATO
[BLUFOR,100,{
    if (isServer && {bomb getVariable "counter" < 5}) then {
        [] execVM "bigBoom.sqf";
        [3] call f_fnc_mpEnd;
    };
...
if (isServer) then {
    [bomb1,bomb2,bomb3,bomb4,bomb5] execVM "bomb.sqf";
};

if (side player == east) then {
    _marker = createMarkerLocal ["cops",getMarkerPos "copsLoc"];
    _marker setMarkerShapeLocal "ICON";
    _marker setMarkerTypeLocal "hd_warning";
    _marker setMarkerColorLocal "ColorRed";
    _marker setMarkerTextLocal "Cops!";
};

if (side player == west) then {
    _marker = createMarkerLocal ["terrorists",getMarkerPos "terroristsLoc"];
    _marker setMarkerShapeLocal "ICON";
