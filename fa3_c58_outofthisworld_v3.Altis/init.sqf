...

{
	if (!isNil {_x getVariable "z"}) then {
		_x setPosATL [getPosATL _x select 0, getPosATL _x select 1, _x getVariable "z"];
...
hololight = {
    ...
    _light setLightAmbient [.5,.5,1];
    _light setLightBrightness 1;
    _light setLightAttenuation [0,1,1,1,1,1];
    waitUntil {sleep 10; !alive _this};
    deleteVehicle _light;
};
[] execVM "vr.sqf";
waitUntil {!isNil "tele_7"};

teleporter = compileFinal preprocessFile "teleporter.sqf";

if (isServer) then {
	_hubTeles = [tele_1, tele_2, tele_3, tele_4, tele_5, tele_6];
	{
		_hubTele = _hubTeles call BIS_fnc_selectRandom;
		_x setVariable ["dest", _hubTele, true];
		_hubTele setVariable ["desc", "Teleporter Hub", true];
		_hubTele setVariable ["dest", _x, true];
		_hubTele setVariable ["broken", false, true];
		_hubTeles = _hubTeles - [_hubTele];
	} forEach [tele, tele_7];
};
tele_8 setVariable ["dest", tele_10];
{
	[_x] call teleporter;
} forEach [tele, tele_1, tele_2, tele_3, tele_4, tele_5, tele_6, tele_7, tele_8,tele_10];

["hubspawn", 5] execVM "populateBase.sqf";
["motherspawn", 7] execVM "populateBase.sqf";

[] execVM "ws_garrisonControl.sqf";

motheruplink addEventHandler ["Killed", {[1] call f_fnc_mpEnd}];
motheruplink addEventHandler ["HandleDamage", {
	if (toLower (_this select 4) in ["democharge_remote_ammo", "democharge_remote_ammo_scripted", "satchelcharge_remote_ammo_scripted","lightningbolt"])
		exitWith {1}; 
	0
}];