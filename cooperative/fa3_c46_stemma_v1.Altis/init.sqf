...
if (isServer) then {
	sleep 40;
	_targetCCount = .2 * ({side _x == east} count allUnits);
	waitUntil {sleep 1; {side _x == east} count allUnits < _targetCCount};
	routed = true;
	["Alert",["CSAT forces in the valley routed!"]] remoteExec ["BIS_fnc_showNotification"];
};
