...
// if (isServer) then {
	// {
		// _x addEventHandler ["GetIn", [[_this select 0, true], "lock", _this select 0, true] call BIS_fnc_MP;
	// } forEach (_bluHelis + _redHelis);
// };

{
	//[_x] execVM "heliSelfRepair.sqf";
...

{
	...
	/*if (isNil ([format ["%1", _x], 3, count (format ["%1", _x])-1] call BIS_fnc_trimString)) then {
		deleteVehicle _x;
	};*/
} forEach (_bluhelis + _redHelis);

if (["_G",format ["%1",player],false] call BIS_fnc_inString) then {
    {
...
{
    if (["_G",format ["%1",_x],false] call BIS_fnc_inString) then {
        _x assignAsCargo nearestObject [position leader _x,"B_Heli_Light_01_F"];
        [_x] orderGetIn true;
    } else {
        _x assignAsDriver nearestObject [position _x,"B_Heli_Light_01_F"];
        [_x] orderGetIn true;
    };
	if (!isPlayer _x && (side _x == west || side _x == east)) then {
