...
if (isServer) then {
	_defendTrigger = createTrigger ["EmptyDetector", getMarkerPos "belfort"];
	_defendTrigger setTriggerActivation ["ANY","PRESENT",true];
	_defendTrigger setTriggerStatements ["{vehicle _x in thisList} count playableUnits == 0","[2,false] call f_fnc_mpEnd",""];
	_defendTrigger setTriggerTimeout [15,15,15,true];
	_defendTrigger setTriggerArea [markerSize "belfort" select 0,markerSize "belfort" select 1,markerDir "belfort",false];
};
if (hasInterface) then {
	_defendTrigger = createTrigger ["EmptyDetector", getMarkerPos "belfort"];
	_defendTrigger setTriggerActivation ["ANY","PRESENT",true];
	_defendTrigger setTriggerStatements ["{vehicle _x in thisList} count playableUnits == 0","['Alert',['Belfort is undefended!']] call BIS_fnc_showNotification",""];
	_defendTrigger setTriggerArea [markerSize "belfort" select 0,markerSize "belfort" select 1,markerDir "belfort",false];
};
