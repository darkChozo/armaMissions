...
if (isServer) then {
	[] spawn {
		_indUnits = [];
		{if (side _x == independent) then {_indUnits pushBack _x}} forEach allUnits;
		while {{alive _x && _x inArea winzone1 || _x inArea winzone2} count _indUnits != 0} do {
			sleep 1;
		};
		
		_bluPoints = 0;
		_redPoints = 0;
		_bluWasCapping = false;
		_redWasCapping = false;
		while {true} do {
			_bluCapping = false;
			_redCapping = false;
			{
				if (_x inArea winzone1 || _x inArea winzone2) then {
					if (side _x == west) then {_bluCapping = true};
					if (side _x == east) then {_redCapping = true};
				};
			} forEach playableUnits;
			
			if (_bluCapping) then {
				if (_redCapping) then { // both capping
					_bluPoints = _bluPoints + 1;
					if (!_bluWasCapping || !_redWasCapping) then {
						["Alert",["Base Contested"]] remoteExec ["BIS_fnc_showNotification"];
					};
				} else { // blu capping
					_redPoints = 0;
					if (!_bluWasCapping || _redWasCapping) then {
						["Alert",["NATO is capturing the base"]] remoteExec ["BIS_fnc_showNotification"];
					};
				};
			} else {
				if (_redCapping) then { // red capping
					_redPoints = _redPoints + 1;
					_bluPoints = 0;
					if (_bluWasCapping || !_redWasCapping) then {
						["Alert",["CSAT is capturing the base"]] remoteExec ["BIS_fnc_showNotification"];
					};
				} else { // no capping
					_redPoints = 0;
					_bluPoints = 0;
					if (_bluWasCapping || _redWasCapping) then {
						["Alert",["Capture interrupted"]] remoteExec ["BIS_fnc_showNotification"];
					};
				};
			};
			
			_bluWasCapping = _bluCapping;
			_redWasCapping = _redCapping;
