...
// BLUFOR > NATO
[BLUFOR,100,{[2,false] call f_fnc_mpEnd}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";
...
if (hasInterface) then {
    [] spawn {
        ...
        [player] execVM "betterSand.sqf";
        player addEventHandler ["Fired",{
            if (_this select 6 isKindOf "IRStrobeBase") then {
        ...
        sleep 4;
        playMusic "Track08_Night_ambient";
        ["","Charkia, Altis"] call ws_fnc_showIntro;
        sleep 6;
        cutText ["","BLACK IN"];
    };
};
...
if (isServer) then {
    ...
    sleep 5; // let the buildings settle
    _units = [];
    {
        _grp = createGroup east;
        _grp createUnit ["O_Soldier_lite_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_F", getMarkerPos _x, [], 0, "NONE"];
        [_grp,100] execVM "patrol.sqf";
        _units append units _grp;
    } forEach (["outerGroup"] call ws_fnc_collectMarkers);
    {
        _grp = createGroup east;
        _grp createUnit ["O_Soldier_LAT_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_F", getMarkerPos _x, [], 0, "NONE"];
        _units append units _grp;
    } forEach (["rocketGroup"] call ws_fnc_collectMarkers);
    {
        _grp = createGroup east;
        _grp createUnit ["O_Soldier_GL_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_GL_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_GL_F", getMarkerPos _x, [], 0, "NONE"];
        [_grp,100] execVM "patrol.sqf";
        _units append units _grp;
    } forEach (["grenadeGroup"] call ws_fnc_collectMarkers);
    {
        _grp = createGroup east;
        _grp createUnit ["O_Soldier_AR_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_lite_F", getMarkerPos _x, [], 0, "NONE"];
        _grp createUnit ["O_Soldier_lite_F", getMarkerPos _x, [], 0, "NONE"];
        _grp setFormDir random 360;
        _grp addWaypoint [getMarkerPos _x,0] setWaypointType "HOLD";
        _units append units _grp;
    } forEach (["innerGroup"] call ws_fnc_collectMarkers);
    _grp = createGroup east;
    banditleader = _grp createUnit ["O_Officer_F", getMarkerPos "leader", [], 0, "CAN_COLLIDE"];
    publicVariable "banditleader";
    _grp createUnit ["O_Soldier_AR_F", getMarkerPos "leader", [], 0, "CAN_COLLIDE"];
    _grp createUnit ["O_Soldier_AR_F", getMarkerPos "leader", [], 0, "CAN_COLLIDE"];
    _grp setFormDir 90;
    _grp addWaypoint [getMarkerPos "leader",0] setWaypointType "HOLD";
    _units append units _grp;
