...
if (hasInterface) then {
    ...
    if (side player == west || side player == civilian) then {
        player addMagazines [["Chemlight_green","Chemlight_red","Chemlight_blue","Chemlight_yellow"] call BIS_fnc_selectRandom,10];
        waitUntil {!isNil "mazegenDone"};
        waitUntil {mazegenDone};
        waitUntil {sleep .5;player distance getMarkerPos "exit" < 3};
        player setDamage 1;
        ["Alert",[format ["%1 finished!",name player]]] remoteExec ["BIS_fnc_showNotification"];
    };
};
