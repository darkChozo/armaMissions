...
[CIVILIAN,30,2,false] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";
...
if (isServer) then {
	meetingLocation = floor(random 3);
	publicVariable "meetingLocation";

	reinforcement = floor(random 4);
};

switch (meetingLocation) do {
	case 0: {
		{deleteMarker _x} forEach (["meeting2","meeting3"] call ws_fnc_collectMarkers);
		{deleteVehicle _x} forEach (["meeting2","meeting3"] call ws_fnc_collectObjects);
	};
	case 1: {
		{deleteMarker _x} forEach (["meeting1","meeting3"] call ws_fnc_collectMarkers);
		{deleteVehicle _x} forEach (["meeting1","meeting3"] call ws_fnc_collectObjects);
	};
	case 2: {
		{deleteMarker _x} forEach (["meeting1","meeting2"] call ws_fnc_collectMarkers);
		{deleteVehicle _x} forEach (["meeting1","meeting2"] call ws_fnc_collectObjects);
	}
};
