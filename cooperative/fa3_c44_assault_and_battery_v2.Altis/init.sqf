...
ugvBackpacks = compileFinal preprocessFileLineNumbers "backpacks.sqf";
expandThisList = compileFinal preprocessFileLineNumbers "expandThisList.sqf";

if (isServer) then {
    _artyLocs = ["arty"] call ws_fnc_collectMarkers;
    for "_i" from 1 to 6 do {
        _loc = _artyLocs call BIS_fnc_selectRandom;
        _arty = createVehicle ["B_MBT_01_mlrs_F", getMarkerPos _loc, [], 0, "CAN_COLLIDE"];
        _arty lock true;
        _arty setDir random 360;
        artys synchronizeObjectsAdd [_arty];
        _grp = createGroup west;
        "B_crew_F" createUnit [getMarkerPos _loc,_grp];
        leader _grp moveInGunner _arty;
        leader _grp execVM "arty.sqf";
        _artyLocs = _artyLocs - [_loc];
    };
};

if (hasInterface) then {
    player addaction ["Set Drop Zone",{
        openMap [true,false];
        onMapSingleClick "createMarker ['dropzone', _pos]; true;";
        waitUntil {getMarkerColor "dropzone" != ""};
        "dropzone" setMarkerShape "ICON";
        "dropzone" setMarkerType "hd_objective";
        "dropzone" setMarkerText "Drop Zone";
        createMarker ["dropZoneArea", getMarkerPos "dropzone"];
        "dropZoneArea" setMarkerShape "ELLIPSE";
        "dropZoneArea" setMarkerSize [300,300];
        "dropZoneArea" setMarkerBrush "Border";
    },"", 10, false,true,"","player == (playableUnits select 0) && getMarkerColor 'dropzone' == ''"];
};

if (isServer) then {
    {
        ([_x,[-0.05,0.05,0.1],"Spine3"] call ws_fnc_attachIR) spawn {
            waitUntil{sleep 5; getPosATL _this select 2 < 400};
            detach _this;
            _pos = position _this;
            deleteVehicle nearestObject [_pos, "NVG_TargetBase"];
            deleteVehicle nearestObject [_pos, "NVG_TargetBase"];
        };
        sleep random .5;
