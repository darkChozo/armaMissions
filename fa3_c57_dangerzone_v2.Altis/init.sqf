...
f_var_doNotRemoveBodies = [Corpses];
...
_bombLocs = [["bombloc","bomberloc"]];
for "_i" from 1 to 14 do {
	_bombLocs = _bombLocs + [[format ["bombloc_%1",_i], format ["bomberloc_%1",_i]]];
};

{
	_loc = _bombLocs call BIS_fnc_selectRandom;
	_bombLocs = _bombLocs - [_loc];
	_bomb = createVehicle ["ModuleExplosive_IEDLandBig_F", getMarkerPos (_loc select 0),[], 0, "CAN_COLLIDE"];
	_bomb setDir random 360;
	_x select 1 setPos position _bomb;
	_x select 0 setPos getMarkerPos (_loc select 1);
	_x select 0 setDir ([_x select 0,_bomb] call BIS_fnc_dirTo);
	_x select 0 setBehaviour "STEALTH";
	_x select 0 setCombatMode "BLUE";
	_x select 0 disableAI "MOVE";
	_x select 0 addAction ["Examine", {hint "You find a trigger for an IED on his body."},[],6,true,true,"","!alive _target and _this distance _target < 2"];
	if (f_var_debugMode == 1) then {
		player sideChat ("ied planted at " + (_loc select 0));
		_loc select 0 setMarkerType "Minefield";
	};
} forEach [[bomber,bombTrigger],[bomber_1,bombTrigger_1],[bomber_2,bombTrigger_2]];

kerryescape attachTo [kerry,[0,0,0]];
...
[] spawn {
	...
	numFIA = 0;
	{
		if (faction _x == "BLU_G_F") then { numFIA = numFIA + 1 };
	} forEach allUnits;
	if (f_var_debugMode == 1) then {player sideChat format ["numFIA = %1", numFIA]};
};

sleep 30; // wait a bit for units to spawn

_aiwound = compileFinal preprocessFile "aiwound.sqf";
{
    if (!isPlayer _x) then {
        _x addEventHandler ["HandleDamage", _aiwound];
        sleep .5;
    };
} forEach allUnits;