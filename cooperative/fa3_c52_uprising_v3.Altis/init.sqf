...
"caches" setMarkerAlpha 0;

[] execVM "ws_garrisonControl.sqf";
[resistance] call ws_fnc_betterVehicle;

broadcastComplete = false;
syrtaTaken = false;

sleep 5;
if (isNil "GrpFIA_CO") then {
	deleteVehicle cacheco;
};
if (isNil "GrpFIA_MMG1") then {
	deleteVehicle cache1mmg;
};
if (isNil "GrpFIA_MMG2") then {
	deleteVehicle cache2mmg;
};
if (isNil "GrpFIA_IFV1") then {
	deleteVehicle VehFIA_IFV1;
};
if (isNil "GrpFIA_IFV2") then {
	deleteVehicle VehFIA_IFV2;
};
