...
[east] call ws_fnc_betterVehicle;

{
	if (side _x == east && !(_x isKindOf"O_T_ghillie_tna_F")) then { _x addPrimaryWeaponItem "optic_ACO_grn" };
} forEach allUnits;

if (isServer) then {
	{
		_trigger = [] call compile _x; //i hate this
		_marker = createMarker [_x,position _trigger];
	...
	} forEach ["westSector","eastSector","southSector"];
};
