...
case "engm":
{
	...
	if (_loadout == 1) then {
		...
		(unitBackpack _unit) addMagazineCargoGlobal ["APERSTripMine_Wire_Mag",1];
	};
	// LOADOUT: HEAVY
	if (_loadout == 2) then {
		_unit addBackpack _baglarge;
		...
		(unitBackpack _unit) addItemCargoGlobal ["ToolKit",1];
		(unitBackpack _unit) addMagazineCargoGlobal ["ClaymoreDirectionalMine_Remote_Mag",1];
		(unitBackpack _unit) addMagazineCargoGlobal ["APERSTripMine_Wire_Mag",2];
		(unitBackpack _unit) addMagazineCargoGlobal ["APERSBoundingMine_Range_Mag",2];
	};
};
