...
{
	if (isServer) then {
		_x setVariable ["flagSwapped",false,true];
	};
	[
		_x,                                                                           // Object the action is attached to
		"Swap Flag",                                                                       // Title of the action
		"flagCapture.paa",                      // Idle icon shown on screen
		"flagCapture.paa",                      // Progress icon shown on screen
		"_this distance _target < 5 && !(_target getVariable 'flagSwapped')",                                                        // Condition for the action to be shown
		"_this distance _target < 5 && !(_target getVariable 'flagSwapped')",                                                      // Condition for the action to progress
		{},                                                                                  // Code executed when action starts
		{},                                                                                  // Code executed on every progress tick
		{																					 // Code executed on completion
			_this select 0 setVariable ["flagSwapped",true,true];
			[_this select 0,"\A3\Data_F\Flags\Flag_AltisColonial_CO.paa"] remoteExec ["setFlagTexture"];
			_mkr = format ["%1",_this select 0];
			_mkr setMarkerColor "ColorGUER";
			["Alert",[format ["%1 Flag Raised",markerText _mkr]]] remoteExec ["BIS_fnc_showNotification"];
		},                                                
