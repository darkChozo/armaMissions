...
// OPFOR > CSAT
// [OPFOR,100,{[2,false] call f_fnc_mpEnd}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";

// INDEPENDENT > AAF
// [INDEPENDENT,100,{[2,false] call f_fnc_mpEnd}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";

[["UnitAAF_CO","GrpAAF_ASL","GrpAAF_A1","GrpAAF_A2","GrpAAF_BSL","GrpAAF_B1","GrpAAF_B2",
  "UnitCSAT_CO","GrpCSAT_ASL","GrpCSAT_A1","GrpCSAT_A2","GrpCSAT_BSL","GrpCSAT_B1","GrpCSAT_B2"],100,{[2,false] call f_fnc_mpEnd}] execVM "f\casualtiesCap\f_CasualtiesCapCheck.sqf";
...
intelScript = compileFinal preprocessFileLineNumbers "intel.sqf";
publicVariable "intelScript";
indforSpotted = false;
intelTracker setVariable ["intelsCollected", 0, true];
intelTracker setVariable ["destroyedCamps", 0, true];
intelTracker setVariable ["intelsLeft", true, true];

_intels = [intel] call ws_fnc_collectObjectsNum;
_camps = [];
_numIntels = var_camps;
if (var_test == 1) then
{
    _numIntels = count _intels;
};
for "_i" from 0 to _numIntels - 1 do {
    _intel = (_intels call BIS_fnc_selectRandom);
    [[_intel, _i], "intelScript", true, true] call BIS_fnc_MP;
    [_intel, _i] execVM "generateCamp.sqf";
    _camps = _camps + [_intel];
    _intels = _intels - [_intel];
};
intelTracker setVariable ["camps", _camps];

_closest = [0,0];
{
    if (position _x distance2D getMarkerPos "aafbase" < _closest distance2D getMarkerPos "aafbase") then {
        _closest = position _x;
    };
} forEach _camps;
_angle = random 360;
_distance = random 350;
createMarker ["nearestCamp",[((random _distance)*cos _angle) + (_closest select 0),((random _distance)*sin _angle) + (_closest select 1)]];
"nearestCamp" setMarkerShape "ELLIPSE";
"nearestCamp" setMarkerSize [400,400];
"nearestCamp" setMarkerColor "ColorRed";
"nearestCamp" setMarkerBrush "SolidBorder";
"nearestCamp" setMarkerAlpha .5;

{
    {
        if (owner _x != -1) then {deleteVehicle _x};
    } forEach (nearestObjects [position _x, [], 250]);
} forEach _intels;

{
    for "_i" from 1 to 5 do {
        _wp = group _x addWaypoint [position (_camps call BIS_fnc_selectRandom),0];
        _wp setWaypointType "MOVE";
        _wp setWaypointCompletionRadius 50;
    };
    _wp = group _x addWaypoint [position _x,0];
    _wp setWaypointType "CYCLE";
} forEach ([patrol] call ws_fnc_collectObjectsNum);

_camps execVM "intelTracker.sqf";
