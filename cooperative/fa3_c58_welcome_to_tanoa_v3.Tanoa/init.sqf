...
player addEventHandler ["Fired",{
	_replacement = "";
	{
		if (_this select 6 isKindOf (_x select 0)) then {
			_replacement = _x select 1;
		};
	} forEach [["Chemlight_green","F_40mm_Green"],
			   ["Chemlight_red","F_40mm_Red"],
			   ["Chemlight_yellow","F_40mm_Yellow"],
			   ["Chemlight_blue","F_40mm_White"]];
	if (_replacement != "") then {
		_vel = velocity (_this select 6);
		_vdir = vectordir (_this select 6);
		_vup = vectorUp (_this select 6);
		_shell = createVehicle [_replacement,position (_this select 6),[],0,"CAN_COLLIDE"];
